...
async function checkSignature(event) {
    ...
    var signature = "<div><b>OK parfait !</b><br/>LaunchEvent OnNewMessageCompose OK</div>";
...
//     }
   ...
   // 5) Insertion signature + event.completed()
      ...
      item.body.setSignatureAsync(
        ...
        function (asyncResult) {
          if (asyncResult.status === Office.AsyncResultStatus.Succeeded) {
            ...
            console.error("Erreur insertion:", asyncResult.error?.message || asyncResult.error);
          }
    ...
    // fallback si setSignatureAsync pas dispo
    item.body.setSelectedDataAsync(signature, { coercionType: Office.CoercionType.Html }, function () {
      if (event) event.completed();
    });
  } else {
    console.log("API body non supportée.");
    if (event) event.completed();
  }
}